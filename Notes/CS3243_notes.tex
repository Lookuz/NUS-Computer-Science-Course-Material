...
Unification is a key component of all first order logic inference algorithms, and it is a process that aims to make 2 logical expressions in FOL language look identical. The \text{\small{UNIFY}} algorithm takes in 2 sentences $p, q$, and returns a unifier $\theta$ for them if one exists:
...
Previously, we have mentioned that the \text{\small{UNIFY}} procedure should return a single substitution that makes the 2 sentences look identical. However, we may have situations where there are multiple possible substitutions that fulfil the unification. In $\text{\small{UNIFY}} (Knows(John, x), Knows(y, z)$, we can use $\theta = \{ x \leftarrow z, y \leftarrow John \}$ as a substitution. But we can also use $\theta = \{x \leftarrow John, y \leftarrow John, z \leftarrow John \}$ since the 2 sentences are still identical. We say that the first unifier is \textit{more general} than the second, since it places less restrictions than the second and yet captures the information that the second unifier implies. In particular, it turns out that for every unifiable pair of expressions, there is a single \textbf{most general unifier(MGU)}, that entails the information of every other possible unifier, and it is unique up to renaming and substitution of variables. Hence, we wish to find the MGUs for each unification process, and the algorithm for doing so can be found below:

\begin{algorithm}[H]
\begin{algorithmic}
\caption{\textsc{\small{UNIFICATION}}}\label{euclid}
\Procedure{\textsc{\small{UNIFY}}}{$x, y, \theta$} \textbf{returns} substitution $\theta$ to make $x, y$ identical
\State \textbf{inputs:} 
...
\ElsIf{$x = y$} \Return{$\theta$}
\ElsIf{$\textsc{\small{VARIABLE?}}(x)$} \Return{$\textsc{\small{UNIFY-VAR}}(x, y, \theta)$}
\ElsIf{$\textsc{\small{VARIABLE?}}(y)$} \Return{$\textsc{\small{UNIFY-VAR}}(y, x, \theta)$}
\ElsIf{$\textsc{\small{COMPOUND?}}(x) \land \textsc{\small{COMPOUND?}}(y)$ } 
\State \Return{\textsc{\small{UNIFY}}(x.\textsc{\small{ARGS}}, y.\textsc{\small{ARGS}}, \textsc{\small{UNIFY}}(x.\textsc{\small{OP}}, y.\textsc{\small{OP}}, $\theta$))}
\ElsIf{$\textsc{\small{LIST?}}(x) \land \textsc{\small{LIST?}}(y)$ } 
\State \Return{\textsc{\small{UNIFY}}(x.\textsc{\small{REST}}, y.\textsc{\small{REST}}, \textsc{\small{UNIFY}}(x.\textsc{\small{FIRST}}, y.\textsc{\small{FIRST}}, $\theta$))}
\Else \ \Return{fail}
\EndIf

\EndProcedure\\

\Procedure{\textsc{\small{UNIFY-VAR}}}{$var, x, \theta$} \textbf{returns} substitution $\theta$
\If{$var \leftarrow val \in \theta$} \Return{\textsc{\small{UNIFY}}(val, x, $\theta$)}
\ElsIf{$var \leftarrow val \in \theta$} \Return{\textsc{\small{UNIFY}}(val, x, $\theta$)} 
\ElsIf{\textsc{\small{OCCUR-CHECK?}}(var, x)} \Return{fail}
...
\State $new \leftarrow \{ \}$
\For{\textbf{each $rule$ in $KB$}}
\State $(p_1 \land ... \land p_n \implies q) \leftarrow \textsc{\small{STANDARDIZE-VARIABLES}}(rule)$
\For{\textbf{each} $\theta$ such that \textsc{\small{SUBST}}($\theta, p_1 \land ... \land p_n$) = \textsc{\small{SUBST}}($\theta, p'_1 \land ... \land p'_n$) for some $p'_1 \land ... \land p'_n$ in $KB$}
...
\begin{algorithmic}[1]
\Procedure{ \textsc{\small{FOL-BC-ASK}}} {$(KB, query)$} \textbf{returns} a generator of substitutions
\State \Return $\textsc{\small{FOL-BC-OR}}(KB, query, \{ \})$
\EndProcedure\\

\Procedure{ \textsc{\small{FOL-BC-OR}}} {$(KB, goal, \theta)$} \textbf{yields} a substitution
\For{\textbf{each} rule $(lhs \implies rhs)$ \textsc{\small{FETCH-RULES-FOR-GOAL}}$(KB, goal)$}
\State $(lhs, rhs) \implies \textsc{\small{STANDARDIZE-VARIABLES}}(lhs, rhs)$
\For{\textbf{each} $\theta'$ in \textsc{\small{FOL-BC-AND}}$(KB, lhs, \textsc{\small{UNIFY}}(rhs, goal, \theta))$}
\State \textbf{yield} $\theta'$
\EndFor
\EndFor
\EndProcedure\\

\Procedure{ \textsc{\small{FOL-BC-AND}}} {$(KB, goal, \theta)$} \textbf{yields} a substitution
\If{$\theta = failure$} \Return 
\ElsIf{\textsc{\small{LENGTH($goals$)}} = 0 } \textbf{yield} $\theta$
\Else
\State $first, rest \leftarrow \text{\small{FIRST}}(goals), \text{\small{REST}}(goals)$
\For{\textbf{each $\theta'$ in} \textsc{\small{FOL-BC-OR}}$(KB, \text{\small{SUBST}}(\theta, first), \theta)$}
\For{\textbf{each $\theta"$ in} \textsc{\small{FOL-BC-AND}}$(KB, rest, \theta')$}
\State \textbf{yield} $\theta"$
\EndFor
\EndFor
\EndIf
...
\begin{center}
\begin{figure}[!htb]
\includegraphics[width=\linewidth]{active-triples.png}
